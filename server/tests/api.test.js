...
test('GET /api/models/state (auth flow)', async () => {
  ...
  process.env.OPENCLAW_USERS_PATH = path.join(dir, 'users.json');
  ...
  const check = await request(app).get('/api/system/selfcheck').set('Authorization', `Bearer ${login.body.token}`);
  assert.equal(check.statusCode, 200);
  assert.ok(check.body.ok);
});
